...
export class EndpointFactory {
  ...
  private readonly _forgotPasswordUrl = "/api/Account/ForgotPassword";
  private readonly _resetPasswordUrl = "/api/Account/ResetPassword";
  ...
  getLoginEndpoint<T>(userName: string, password: string): Observable<T> {
    ...
    var base = this.configurations.baseUrl
    return this.http.post<T>('connect/token', requestBody, { headers: header });
  }

  getForgotPasswordEndPoint<T>(email: string) {
    let header = new HttpHeaders({ 'Content-Type': 'application/json; charset=UTF-8' });
    //let params = new HttpParams()
    //  .append('Email', email);
    //let requestBody = params.toString();
    return this.http.post<T>('api/Account/ForgotPassword'+'/'+email, null, { headers : header });
  }

  resetPasswordEndPoiint<T>(resetPasswordViewModel : ResetPasswordViewModel) {
    let header = new HttpHeaders({ 'Content-Type': 'application/json; charset=UTF-8' });
    return this.http.post<T>('api/Account/ResetPassword', resetPasswordViewModel, { headers : header });
  }
