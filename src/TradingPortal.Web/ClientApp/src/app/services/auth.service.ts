...
export class AuthService {
  ...
  public invalidCredentials: (err: HttpErrorResponse) => void;
  public logoutDelegate: () => void;
  public storeCloseDelegate: (isStoreClosed: Boolean) => void;
  ...
  private _isPortalClosed: boolean = false;
  ...
  forgotPassword(email : string) {
    return this.endpointFactory.getForgotPasswordEndPoint<string>(email).pipe(
      map(response => response));
  }

  resetPassword(resetPasswordViewModel : ResetPasswordViewModel) {
    return this.endpointFactory.resetPasswordEndPoiint<IdentityResultCore>(resetPasswordViewModel).pipe(
    map(response => response))
  }
  ...
  private processLoginResponse(response: LoginResponse, rememberMe: boolean) {
    ...
    user.isEnabled = true;
    if (decodedIdToken.configuration == 'storeclosed') {
      this._isPortalClosed = true;
    }
  ...
  get isPortalClosed(): boolean {
    return this._isPortalClosed;
  }

  set isPortalClosed(isPortalClosed : boolean) {
    this._isPortalClosed = isPortalClosed
  }

  setPortalClosedStatus(isPortalClosed: boolean) {
    this._isPortalClosed = isPortalClosed;
    this.storeCloseDelegate(isPortalClosed);
  }

  isInRole(roleName: string): boolean {
    let user = this.localStorage.getDataObject<User>(DBkeys.CURRENT_USER);
    if (user && user.roles.indexOf(roleName) > -1) {
      return true;
    }
    return false;
  }
