...
import {UsersGridCommand } from '../models/user.model';
...
export class AccountEndpoint extends EndpointFactory {
  ...
  getUsersEndpointForGrid<T>(command : UsersGridCommand): Observable<T> {
    //let endpointUrl = page && pageSize ? `${this.usersUrl}/${page}/${pageSize}` : this.usersUrl;
    let endpointUrl = this.usersUrl;
    return this.http.post<T>(endpointUrl,command, this.getRequestHeaders()).pipe<T>(
      catchError(error => {
        return this.handleError(error, () => this.getUsersEndpointForGrid(command));
      }));
  }
